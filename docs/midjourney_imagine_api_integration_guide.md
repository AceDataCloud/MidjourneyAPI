...
First, we need to obtain the mask for that area, which is derived from a grayscale image encoded in Base64. Below are some example codes for obtaining the mask:
...
To provide a better user experience, this API supports streaming output, meaning that when "image generation starts," it begins to return results. Whenever there is a change in drawing progress, the results will be streamed until the image generation ends.
