...
This API also supports local drawing functionality for images, but only for images generated under the previous content. We can input the unique ID of a generated image, the action parameter for local repainting, and the mask for the area to be repainted, to achieve repainting in the masked area.
...
It can be seen that the image in the mask area has been redrawn, and the returned result is consistent with the above content, as shown in the figure below:
