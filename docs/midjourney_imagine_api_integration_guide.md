...
Additionally, you can combine the streaming results with asynchronous callbacks by specifying the request header `accept=application/x-ndjson` and the request body of callback_url, then callback_url can receive multiple POST requests of streaming results.